...
`active`; categories are optional, but a product with no category does **not** appear on the
public/live menu until assigned to one. Nest `option_groups[].items[]` on each product when
complements are known. Prefer these over looping `create_product` / `create_category`.
Single-product conversational alta still uses the secretary flow.
