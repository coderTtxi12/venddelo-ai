...
2. tools de menu_read                ← categorías, productos, complementos y promos en vivo;
                                      céntrate en lo que el cliente final ve (nombres,
...
**Regla estricta — sin afirmaciones sin leer.** Nunca digas que un producto tiene
...
o complementos es inventar datos — prohibido. Si aún no leíste productos, léelos antes de
...
Un menú digital con **nombres completos y descripciones** puede
**aumentar la conversión hasta ~90%**.
Los clientes deciden por lo que leen — no solo por el menú físico.
...
| "Mejora descripciones / orden" | Activa **`menu_read`**, **`menu_write`** y esta skill → propone cambios alineados |
...
| Producto / platillo | `products` (`name`, `description`, `price_cents`, M:N categories) | `list_products`, `get_product`, `create_product`, `update_product` |
...
| Promo de marketing (2×1) | `promotions` type `bundle` (NxM), scope product/category | `create_promotion`, `update_nxm_promotion`, `update_nxm_promotion_complements` (`load_skill(promotions)`) |
...
| Tema del menú digital | branding de `restaurants` | `menu_write` `list_menu_themes`, `apply_menu_theme` |
...
|-----------|--------------------------|
| **Promociones** (1.er lugar) | Está habilitada y hay al menos una promo de marketing activa con productos ligados |
...
- **`grid`** — bebidas, postres, pocos items pero importante resaltarlos
- **`horizontal`** — carrusel destacado

Ejemplo: categoría Bebidas en **`grid`** mejora el escaneo del menú.
...
## 3. Complementos (grupos y opciones)
...
- Para "15% off en estos productos" → `apply_product_discount` (descuento de catálogo, no campaña de marketing).
...
## 6. Branding de la tienda

Nombre y descripción del restaurante generan confianza en la vitrina Venddelo.

- **`update_restaurant`**: `name`, `description` (tema del menú digital:
...
y promociones (`categories`, `products`, `option_groups`, `promotions`,
