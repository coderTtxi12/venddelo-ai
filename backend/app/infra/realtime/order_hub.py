...
class OrderRealtimeHub:
    """In-process WebSocket fan-out for kitchen order events."""
    ...
    async def connect(self, restaurant_id: uuid.UUID, websocket: WebSocket) -> None:
        ...
        logger.info("orders ws connected restaurant_id=%s", restaurant_id)
    ...
    async def disconnect(self, restaurant_id: uuid.UUID, websocket: WebSocket) -> None:
        ...
        logger.info("orders ws disconnected restaurant_id=%s", restaurant_id)

    def publish_sync(self, restaurant_id: uuid.UUID, payload: dict[str, Any]) -> None:
        if self._loop is None or self._queue is None:
            logger.debug("orders ws hub not started; dropping event restaurant_id=%s", restaurant_id)
            return
        room = self._room_key(restaurant_id)
    ...
    async def _broadcast(self, room: str, payload: dict[str, Any]) -> None:
        ...
        for socket in sockets:
            ...
            try:
                ...
                logger.warning("orders ws send failed", exc_info=True)
                stale.append(socket)
        for socket in stale:
            self._rooms[room].discard(socket)


_hub = OrderRealtimeHub()


def get_order_realtime_hub() -> OrderRealtimeHub:
    return _hub
