...
class TrackingRealtimeHub:
    """In-process WebSocket fan-out for public dispatch tracking links."""
    ...
    async def connect(self, token: str, websocket: WebSocket) -> None:
        ...
        self._rooms[self._room_key(token)].add(websocket)
        logger.info("tracking ws connected token=%s", token[:8])

    async def disconnect(self, token: str, websocket: WebSocket) -> None:
        room = self._room_key(token)
        self._rooms[room].discard(websocket)
        if not self._rooms[room]:
            del self._rooms[room]
        logger.info("tracking ws disconnected token=%s", token[:8])

    def publish_sync(self, token: str, payload: dict[str, Any]) -> None:
        if self._loop is None or self._queue is None:
            logger.debug("tracking ws hub not started; dropping event token=%s", token[:8])
            return
        room = self._room_key(token)
    ...
    async def _broadcast(self, room: str, payload: dict[str, Any]) -> None:
        ...
        for socket in sockets:
            ...
            try:
                ...
                logger.warning("tracking ws send failed", exc_info=True)
                stale.append(socket)
        for socket in stale:
            self._rooms[room].discard(socket)


_hub = TrackingRealtimeHub()


def get_tracking_realtime_hub() -> TrackingRealtimeHub:
    return _hub
