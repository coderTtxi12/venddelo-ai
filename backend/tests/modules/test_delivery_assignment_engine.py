...
def test_case_c_skips_other_restaurant():
    ...
    hooked = _driver(
        ...
        active_request_status="assigned",
        active_package_count=1,
        assigned_restaurant_ids=("rest-other",),
...
def test_missing_app_build_is_not_assigned():
    ...
    fresh = _driver("fresh", last_lat=19.4400, last_lng=-99.1400, app_build_number=3)
...
def test_stale_app_build_is_not_assigned():
    request = _request()
    stale = _driver("stale", last_lat=19.4330, last_lng=-99.1335, app_build_number=2)
    fresh = _driver("fresh", last_lat=19.4400, last_lng=-99.1400, app_build_number=3)
    result = choose_assignments(_context(request, (stale, fresh)))
...
def test_eligibility_blockers_outdated_app():
    ...
    stale = _driver("stale", last_lat=19.4330, last_lng=-99.1335, app_build_number=2)
    current = _driver("current", last_lat=19.4330, last_lng=-99.1335, app_build_number=3)
