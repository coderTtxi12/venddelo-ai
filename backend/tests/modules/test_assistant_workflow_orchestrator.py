...
def test_workflow_orchestrator_menu_import_handoff_skips_executor():
    ...
    orchestrator = WorkflowOrchestrator(
        settings=settings,
        rollout_skill_ids=("menu_read", "menu_import"),
    )
    runtime = _runtime_bundle()
    runtime = WorkflowRuntimeBundle(
        context=WorkflowContext(
            user_message="Quiero importar mi menú",
            restaurant_id=runtime.context.restaurant_id,
            conversation_id=runtime.context.conversation_id,
            effective_skill_ids=["menu_read", "menu_import"],
            skill_catalog=runtime.context.skill_catalog,
            system_prompt=runtime.context.system_prompt,
            conversation_history=runtime.context.conversation_history,
            assistant_display_name=runtime.context.assistant_display_name,
        ),
        registry=runtime.registry,
        conversation_id=runtime.conversation_id,
    )

    import_plan = WorkflowPlan(
        goal="Importar menú desde archivos",
        route="menu_import",
        requires_tools=True,
        steps=[],
        success_criteria=["El flujo de importación avanza"],
    )
    import_execution = ExecutionRecord(
        summary="Extraje 12 productos y comparé con el menú live.",
        tools_used=["start_menu_extraction_batch", "analyze_import_vs_live"],
    )
    ...
    async def fake_run(agent, agent_input, context=None, max_turns=1):  # noqa: ARG001
        name = getattr(agent, "name", "")
        run_agents.append(name)
        if name == "Planner":
            return _structured_result(import_plan)
        if name == "MenuImport":
            return _run_result(import_execution.summary)
        raise AssertionError(f"Unexpected agent run: {name!r}")
    ...
    with (
        ...
        events = asyncio.run(_collect(orchestrator, message="Importa mi menú"))

    phases = [event.data["phase"] for event in events if event.event == "agent.phase"]
    assert phases == ["context", "planning", "menu_import", "responding"]
    assert run_agents == ["Planner", "MenuImport"]
    assert not any(event.event == "agent.evaluation" for event in events)
