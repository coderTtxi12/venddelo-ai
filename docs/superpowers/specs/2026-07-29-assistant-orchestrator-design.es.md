...
3. `delegate_task` ejecuta exactamente uno de:
   - `menu_import_subagent` (antes Menu Import Executor)
...
Orchestrator Agent ── texto libre (Markdown ES) ──► content.delta → usuario
  ...
  └─ menu_import_subagent     → ExecutionRecord (JSON) → vuelve al Orchestrator
         │
...
| MenuImportExecutor | `menu_import_subagent` |
...
- **Responsabilidades:**
  ...
  - Delegar a `menu_import_subagent` solo cuando aplique onboarding/import (capability + intención o sesión activa).
...
```text
delegate_task(
  subagent: "restaurant_ops_subagent" | "menu_import_subagent",
...
1. Validar `subagent`.
2. Si `menu_import_subagent` y menu import no está enabled / sin registry → devolver error claro en el tool result (el Orchestrator no inventa import).
...
- Si hoy se inyectaba `public_menu_url` al MenuImportResponder, anexarlo al **tool result** de `delegate_task` tras `menu_import_subagent` cuando aplique la misma condición `should_inject_public_menu_url_for_responder` (el Orchestrator lo usa al redactar; no se inventa).
...
5. Si en el turn se usó `menu_import_subagent` (o hay sesión con quiz pendiente al final): emitir quiz SSE desde DB.
...
- **Orchestrator:** síntesis de:
  ...
  - Cuándo elegir `restaurant_ops_subagent` vs `menu_import_subagent` (misma lógica de negocio que el Router actual + atención a sesión de import / aclaraciones).
...
- **menu_import_subagent:** basado en `MENU_IMPORT_EXECUTOR_INSTRUCTIONS`, summary orientado al Orchestrator (no al Responder).
...
2. **Workflow (mocks de Runner):**
   ...
   - Una delegación `menu_import_subagent` → tool SSE + respuesta + quiz SSE cuando la sesión tiene pending questions.
